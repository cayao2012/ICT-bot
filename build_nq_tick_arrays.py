#!/usr/bin/env python3
"""
Build NQ tick arrays (NQ_prices.npy, NQ_times.npy) from dbn.zst files.

Two types of source files:
1. Per-contract: glbx-mdp3-YYYYMMDD.trades.NQH5.dbn.zst (already NQ-specific)
2. Parent symbol: glbx-mdp3-YYYYMMDD.trades.dbn.zst (contains all instruments, filter to NQ)

For each day, picks the highest-volume NQ outright contract (front month).

Output:
- NQ_prices.npy (float64) - tick prices sorted by ts_recv
- NQ_times.npy  (int64)   - ts_recv in nanoseconds
"""
...
OUT_DIR  = "/Users/gtrades/ptnut-bot/data_new_raw"

# ── Step 1: Discover files ──────────────────────────────────────────────────

# Per-contract NQ files (NQH5, NQM5, etc. — no spreads)
per_contract_pattern = os.path.join(DATA_DIR, "glbx-mdp3-*.trades.NQ??.dbn.zst")
per_contract_files = sorted(glob.glob(per_contract_pattern))
# Filter out any spread files that might match (shouldn't with NQ?? but be safe)
per_contract_files = [f for f in per_contract_files if "-NQ" not in os.path.basename(f).split(".trades.")[1]]
print(f"Per-contract NQ files found: {len(per_contract_files)}")
...
for f in per_contract_files:
    m = re.search(r"glbx-mdp3-(\d{8})\.trades\.(NQ[A-Z]\d)\.dbn\.zst$", os.path.basename(f))
    if m:
        date_str = m.group(1)
...
print("\n── Processing per-contract NQ files ──")
for date_str in sorted(date_to_per_contract.keys()):
    ...
    for fpath in fpaths:
        ...
        m = re.search(r"\.trades\.(NQ[A-Z]\d)\.dbn\.zst$", fname)
...
for fpath in parent_files_to_use:
    ...
    try:
        ...
        # Filter to NQ outright symbols (no spreads)
        if "symbol" in df.columns:
            ...
            # Pick highest-volume NQ contract
...
prices_path = os.path.join(OUT_DIR, "NQ_prices.npy")
times_path  = os.path.join(OUT_DIR, "NQ_times.npy")
